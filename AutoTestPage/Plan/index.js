...
import { Form, Switch, Button, Input, Icon, Tooltip, Checkbox, InputNumber, message } from "antd";
...
export default class Add extends Component {
  ...
  // 打开一个新窗口执行一次计划
  runPlan = () => {
    const { form, planMsg } = this.props;
    form.validateFields(async (err, values) => {
      if (!err) {
        try {
          // 走插件后端的 API（调用 schedule.runOnce -> handlerPlan）
          const res = await axios.post('/api/plugin/test/run_once', {
            plan_id: planMsg._id
          });
